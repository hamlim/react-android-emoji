"use strict";

Object.defineProperty(exports, "__esModule", {
  value: true
});

var _react = require("react");

var _react2 = _interopRequireDefault(_react);

function _interopRequireDefault(obj) {
  return obj && obj.__esModule ? obj : { default: obj };
}

function _extends() {
  _extends =
    Object.assign ||
    function(target) {
      for (var i = 1; i < arguments.length; i++) {
        var source = arguments[i];
        for (var key in source) {
          if (Object.prototype.hasOwnProperty.call(source, key)) {
            target[key] = source[key];
          }
        }
      }
      return target;
    };
  return _extends.apply(this, arguments);
}

function SymbolsOverMouth(props) {
  return _react2.default.createElement(
    "svg",
    _extends(
      {
        enableBackground: "new 0 0 128 128",
        viewBox: "0 0 128 128"
      },
      props
    ),
    _react2.default.createElement(
      "symbol",
      {
        viewBox: "-64.5 -64.5 129 129"
      },
      _react2.default.createElement(
        "g",
        {
          fill: "none",
          stroke: "#4285F4",
          strokeMiterlimit: 10
        },
        _react2.default.createElement("rect", {
          x: -64,
          y: -64,
          width: 128,
          height: 128
        }),
        _react2.default.createElement("path", {
          d:
            "M36.95,37.82C27.32,46.32,14.2,51,0,51 c-14.27,0-27.39-4.62-36.96-13.01C-47.45,28.79-53,15.65-53,0c0-15.58,5.55-28.69,16.04-37.92C-27.36-46.35-14.24-51,0-51 c14.17,0,27.29,4.71,36.95,13.25C47.45-28.45,53-15.4,53,0C53,15.47,47.45,28.55,36.95,37.82z"
        }),
        _react2.default.createElement("path", {
          d:
            "m0 55c-29.59 0-57-19.01-57-55 0-35.8 27.41-55 57-55s57 19.69 57 55c0 35.51-27.41 55-57 55z"
        }),
        _react2.default.createElement("path", {
          d:
            "M0-43c-12.29,0-23.54,3.94-31.68,11.09 C-40.39-24.25-45-13.21-45,0c0,29.7,22.6,43,45,43c21.67,0,45-13.46,45-43S21.67-43,0-43L0-43z"
        }),
        _react2.default.createElement("line", {
          x1: -0.01,
          x2: -0.01,
          y1: 51,
          y2: -51
        }),
        _react2.default.createElement("line", {
          x1: -16,
          x2: -16,
          y1: 48.95,
          y2: -48.93
        }),
        _react2.default.createElement("line", {
          x1: 15.99,
          x2: 15.99,
          y1: 48.91,
          y2: -48.93
        }),
        _react2.default.createElement("line", {
          x1: 53,
          x2: -53,
          y1: 0.08,
          y2: 0.08
        })
      )
    ),
    _react2.default.createElement(
      "linearGradient",
      {
        id: "a",
        x1: 65.667,
        x2: 65.667,
        y1: 19.467,
        y2: 111.47,
        gradientUnits: "userSpaceOnUse"
      },
      _react2.default.createElement("stop", {
        stopColor: "#F15B52",
        offset: 0.3413
      }),
      _react2.default.createElement("stop", {
        stopColor: "#EF5851",
        offset: 0.5564
      }),
      _react2.default.createElement("stop", {
        stopColor: "#EA4E50",
        offset: 0.718
      }),
      _react2.default.createElement("stop", {
        stopColor: "#E13E4D",
        offset: 0.8618
      }),
      _react2.default.createElement("stop", {
        stopColor: "#D9314B",
        offset: 0.9457
      })
    ),
    _react2.default.createElement("path", {
      d:
        "m65.67 119.46c-27.88 0-57.9-17.46-57.9-55.79s30.01-55.79 57.89-55.79c15.46 0 29.76 5.12 40.29 14.41 11.52 10.17 17.61 24.48 17.61 41.38 0 16.83-6.09 31.12-17.61 41.31-10.55 9.33-24.86 14.48-40.28 14.48z",
      fill: "url(#a)"
    }),
    _react2.default.createElement("path", {
      d:
        "m65.67 9.98c14.94 0 28.75 4.93 38.9 13.88 11.05 9.75 16.89 23.52 16.89 39.81 0 16.21-5.84 29.95-16.9 39.73-10.17 9-23.98 13.95-38.89 13.95-14.99 0-28.8-4.89-38.9-13.77-11.05-9.71-16.89-23.51-16.89-39.91 0-16.48 5.84-30.3 16.88-39.99 10.07-8.83 23.89-13.7 38.91-13.7m0-4.21c-31.15 0-60 20.01-60 57.89 0 37.68 28.85 57.89 60 57.89s60-20.73 60-57.89c0-37.37-28.85-57.89-60-57.89z",
      fill: "#C7203A"
    }),
    _react2.default.createElement(
      "g",
      {
        fill: "#242424"
      },
      _react2.default.createElement("path", {
        d:
          "m57.87 67.23c-0.69 0-1.35-0.25-1.89-0.72-2.98-2.55-12.74-7.93-16.33-9.69-1.34-0.65-1.98-1.97-1.67-3.32 0.31-1.34 1.45-2.27 2.79-2.27 0.25 0 0.5 0.03 0.75 0.1 1.32 0.34 2.61 0.8 3.85 1.37 3.33 1.56 12.56 6.98 14.8 9.8 0.71 0.9 0.85 2.04 0.36 3.05-0.5 1.01-1.54 1.68-2.66 1.68z"
      }),
      _react2.default.createElement("path", {
        d:
          "m75.67 67.23c0.69 0 1.35-0.25 1.89-0.72 2.98-2.55 12.74-7.93 16.33-9.69 1.34-0.65 1.98-1.97 1.67-3.32-0.31-1.34-1.45-2.27-2.79-2.27-0.25 0-0.5 0.03-0.75 0.1-1.32 0.34-2.61 0.8-3.85 1.37-3.33 1.56-12.56 6.98-14.8 9.8-0.71 0.9-0.85 2.04-0.36 3.05 0.5 1.01 1.54 1.68 2.66 1.68z"
      }),
      _react2.default.createElement("path", {
        d:
          "m43.04 68.84c0 4.29 3.36 7.77 7.5 7.77s7.5-3.48 7.5-7.77v-1.46c0-4.29-3.36-7.77-7.5-7.77s-7.5 3.48-7.5 7.77v1.46z"
      }),
      _react2.default.createElement("path", {
        d:
          "m75.5 68.84c0 4.29 3.36 7.77 7.5 7.77s7.5-3.48 7.5-7.77v-1.46c0-4.29-3.36-7.77-7.5-7.77s-7.5 3.48-7.5 7.77v1.46z"
      })
    ),
    _react2.default.createElement("rect", {
      transform: "matrix(.9945 .1046 -.1046 .9945 10.628 -6.1532)",
      x: 24.26,
      y: 81.36,
      width: 79.43,
      height: 33.75
    }),
    _react2.default.createElement(
      "g",
      {
        fill: "#fff"
      },
      _react2.default.createElement("path", {
        d:
          "m40.39 98.96-2.1-0.2-1.39 5.26-2.59-0.25 1.39-5.26-2.89-0.28 0.23-2.39 3.28 0.32 0.91-3.39-2.88-0.28 0.23-2.39 3.27 0.32 1.42-5.34 2.58 0.25-1.41 5.34 2.1 0.2 1.42-5.34 2.59 0.25-1.42 5.34 3.01 0.29-0.23 2.39-3.4-0.33-0.91 3.39 2.99 0.29-0.23 2.39-3.38-0.33-1.39 5.26-2.59-0.25 1.39-5.26zm-1.47-2.55 2.1 0.2 0.91-3.39-2.1-0.2-0.91 3.39z"
      }),
      _react2.default.createElement("path", {
        d:
          "m58.62 101.17c0.06-0.6-0.04-1.09-0.31-1.46-0.26-0.37-0.76-0.75-1.47-1.12-0.72-0.38-1.37-0.75-1.98-1.13-0.6-0.37-1.11-0.79-1.53-1.24s-0.72-0.96-0.91-1.53c-0.19-0.56-0.25-1.23-0.18-2 0.12-1.29 0.66-2.3 1.61-3.05s2.15-1.12 3.61-1.12l0.23-2.41 1.81 0.18-0.24 2.45c1.37 0.34 2.41 1.02 3.12 2.04s0.99 2.25 0.85 3.7l-3.86-0.37c0.08-0.79-0.02-1.41-0.28-1.87s-0.68-0.71-1.24-0.77c-0.5-0.05-0.91 0.07-1.22 0.36s-0.49 0.71-0.54 1.27 0.06 1.03 0.35 1.39 0.77 0.73 1.45 1.1 1.32 0.75 1.91 1.13c0.6 0.38 1.1 0.8 1.52 1.26 0.42 0.45 0.73 0.97 0.93 1.54 0.21 0.57 0.27 1.24 0.2 2.01-0.13 1.3-0.64 2.32-1.55 3.07-0.9 0.74-2.08 1.11-3.51 1.11l-0.22 2.3-1.81-0.18 0.22-2.31c-1.64-0.34-2.86-1.03-3.65-2.08s-1.11-2.36-0.96-3.94l3.85 0.37c-0.08 0.87 0.04 1.55 0.39 2.04 0.34 0.49 0.86 0.77 1.57 0.84 0.52 0.05 0.94-0.06 1.27-0.34 0.33-0.29 0.52-0.7 0.57-1.24z"
      }),
      _react2.default.createElement("path", {
        d:
          "m67.42 106.35c0.06-0.62 0.33-1.11 0.82-1.47s1.07-0.5 1.74-0.44c0.67 0.07 1.22 0.32 1.63 0.76s0.59 0.97 0.53 1.6c-0.06 0.62-0.34 1.11-0.82 1.47-0.49 0.36-1.07 0.5-1.74 0.43-0.67-0.06-1.21-0.32-1.62-0.76-0.43-0.43-0.6-0.97-0.54-1.59zm4.48-3.51-3.44-0.33 0.67-12.59 4.53 0.44-1.76 12.48z"
      }),
      _react2.default.createElement("path", {
        d:
          "m97.93 100.84c-0.28 2.04-0.94 3.61-2 4.74-1.06 1.12-2.37 1.61-3.93 1.46-0.64-0.06-1.2-0.26-1.66-0.58-0.47-0.33-0.81-0.76-1.04-1.29-0.87 0.96-1.95 1.38-3.24 1.25-0.75-0.07-1.39-0.36-1.91-0.85-0.52-0.5-0.87-1.17-1.06-2-0.19-0.84-0.17-1.77 0.03-2.8 0.42-1.93 1.21-3.45 2.35-4.54s2.46-1.56 3.94-1.42c1.15 0.11 2.12 0.47 2.89 1.09l0.7 0.55-1.27 6.84c-0.24 1.08 0.09 1.66 0.97 1.74 0.66 0.07 1.26-0.31 1.8-1.11 0.53-0.81 0.89-1.87 1.06-3.19 0.39-2.6 0.04-4.67-1.04-6.22-1.08-1.54-2.79-2.43-5.14-2.66-1.49-0.14-2.86 0.11-4.1 0.76s-2.26 1.64-3.07 2.97-1.35 2.89-1.61 4.66c-0.39 2.73-0.04 4.88 1.04 6.45s2.84 2.47 5.29 2.71c0.66 0.06 1.36 0.05 2.11-0.03s1.37-0.2 1.85-0.36l0.25 1.99c-0.51 0.27-1.18 0.46-2.01 0.57s-1.65 0.13-2.46 0.05c-2.14-0.21-3.9-0.79-5.3-1.75s-2.38-2.28-2.94-3.95c-0.57-1.67-0.7-3.65-0.4-5.93 0.3-2.15 0.99-4.04 2.06-5.66s2.43-2.83 4.1-3.61c1.66-0.78 3.48-1.08 5.45-0.89s3.64 0.76 4.99 1.72c1.36 0.96 2.32 2.24 2.89 3.84 0.55 1.62 0.69 3.43 0.41 5.45zm-11.87 0.2c-0.19 1.01-0.18 1.79 0.02 2.33s0.57 0.84 1.12 0.9c0.37 0.03 0.72-0.1 1.06-0.4s0.64-0.74 0.9-1.34l0.99-5.31c-0.23-0.09-0.47-0.14-0.72-0.16-0.88-0.08-1.59 0.19-2.13 0.83-0.54 0.65-0.95 1.7-1.24 3.15z"
      })
    )
  );
}

exports.default = SymbolsOverMouth;
