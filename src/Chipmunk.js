import React from "react";

function Chipmunk(props) {
  return (
    <svg enableBackground="new 0 0 128 128" viewBox="0 0 128 128" {...props}>
      <linearGradient
        id="i"
        x1={-65.161}
        x2={-65.161}
        y1={889.77}
        y2={977.91}
        gradientTransform="matrix(.9558 .0962 -.0841 .9845 235.87 -838.06)"
        gradientUnits="userSpaceOnUse"
      >
        <stop stopColor="#C85C10" offset={0} />
        <stop stopColor="#C45A10" offset={0.236} />
        <stop stopColor="#B9550F" offset={0.4584} />
        <stop stopColor="#A64C0D" offset={0.6754} />
        <stop stopColor="#8B400B" offset={0.8877} />
        <stop stopColor="#7A380A" offset={1} />
      </linearGradient>
      <path
        d="m91.43 117.34c-8.37 0-14.06-4.87-15.77-6.54 1.7-2.46 7.38-10.79 8.36-13.95 1.79-5.76 1.52-12.17-0.76-18.06-0.84-2.18-2.05-3.99-3.22-5.74-0.44-0.66-0.89-1.33-1.33-2.03-1.17-1.88-2.22-4.54-3.03-7.7-0.92-3.6-1.14-7.4-0.64-11 0.76-5.45 3.1-10.54 6.68-14.57l0.02 0.85c0.01 0.4 0.25 0.75 0.61 0.9 0.12 0.05 0.26 0.08 0.39 0.08 0.25 0 0.5-0.09 0.69-0.27 4.95-4.68 11.72-7.36 18.56-7.36 0.58 0 1.17 0.02 1.75 0.06l-0.53 0.9c-0.18 0.31-0.18 0.68-0.01 0.99s0.5 0.5 0.85 0.51c5.1 0.11 10.11 3.1 12.78 7.62 2.1 3.57 2.71 8.03 1.75 12.06-1.01-2.24-2.92-4.05-5.25-4.78-0.76-0.24-1.56-0.36-2.38-0.36-2.88 0-5.61 1.47-7.13 3.84-1.99 3.1-1.75 7.45 0.58 10.34 0.19 0.24 0.48 0.37 0.78 0.37 0.07 0 0.15-0.01 0.22-0.03 0.37-0.09 0.67-0.38 0.75-0.75l0.17-0.75c4.61 5.51 6.52 13.29 4.89 20.34-0.1 0.42 0.09 0.86 0.46 1.08 0.16 0.09 0.34 0.14 0.51 0.14 0.24 0 0.47-0.08 0.66-0.25 0.35-0.31 0.7-0.56 1.04-0.8 0.8 15.05-0.99 30.4-17.21 34.21-1.74 0.44-3.51 0.65-5.24 0.65z"
        fill="url(#i)"
      />
      <path
        d="m101.99 32.96h0.15c-0.12 0.48-0.05 1 0.2 1.45 0.35 0.62 0.99 1.01 1.7 1.02 4.76 0.1 9.45 2.9 11.94 7.13 1.54 2.61 2.22 5.73 2.02 8.77-1.15-1.36-2.65-2.42-4.35-2.96-0.86-0.27-1.76-0.41-2.68-0.41-3.21 0-6.27 1.65-7.97 4.3-2.22 3.45-1.95 8.29 0.64 11.5 0.38 0.48 0.96 0.75 1.56 0.75 0.15 0 0.3-0.02 0.45-0.05 0.36-0.08 0.69-0.27 0.95-0.52 3.67 5.15 5.1 11.95 3.66 18.16-0.2 0.85 0.18 1.72 0.92 2.17 0.32 0.19 0.67 0.28 1.03 0.28 0.27 0 0.53-0.05 0.78-0.16 0.53 14.02-1.66 27.85-16.53 31.35-1.68 0.4-3.38 0.6-5.03 0.6-7.17 0-12.32-3.78-14.45-5.69 2.17-3.16 7.05-10.45 8-13.51 1.85-5.97 1.58-12.62-0.78-18.71-0.88-2.29-2.13-4.14-3.33-5.93-0.43-0.65-0.88-1.32-1.31-2-1.12-1.79-2.12-4.36-2.91-7.42-0.89-3.47-1.1-7.14-0.62-10.61 0.63-4.53 2.48-8.92 5.21-12.54 0.19 0.22 0.44 0.39 0.72 0.51 0.25 0.1 0.51 0.16 0.77 0.16 0.5 0 0.99-0.19 1.37-0.55 4.78-4.51 11.3-7.09 17.89-7.09m0-2c-7.05 0-14.07 2.74-19.25 7.63-0.02-1.08-0.05-2.16-0.07-3.24-4.63 4.37-7.72 10.42-8.61 16.84-0.53 3.78-0.28 7.68 0.66 11.38 0.68 2.65 1.7 5.66 3.15 7.99 1.59 2.56 3.35 4.72 4.47 7.59 2.13 5.51 2.48 11.77 0.74 17.4-1.1 3.53-8.7 14.34-8.7 14.34s6.43 7.44 17.06 7.44c1.72 0 3.55-0.2 5.49-0.65 17.91-4.22 18.86-22.24 17.86-37.21-0.64 0.76-1.58 1.2-2.58 2.08 1.84-7.97-0.68-16.89-6.41-22.65l-0.6 2.61c-2.04-2.53-2.26-6.45-0.51-9.17 1.36-2.12 3.82-3.38 6.29-3.38 0.7 0 1.4 0.1 2.08 0.32 3.05 0.96 5.33 4.12 5.34 7.4 2.43-5.01 2.16-11.32-0.68-16.14s-8.17-7.99-13.62-8.11c0.44-0.75 0.87-1.5 1.31-2.25-1.15-0.15-2.29-0.22-3.42-0.22z"
        fill="#99440B"
      />
      <linearGradient
        id="h"
        x1={46.066}
        x2={46.066}
        y1={126.52}
        y2={108.57}
        gradientTransform="matrix(.9982 .0595 -.0595 .9982 7.0563 -2.9461)"
        gradientUnits="userSpaceOnUse"
      >
        <stop stopColor="#C85C10" offset={0} />
        <stop stopColor="#C65B10" offset={0.4728} />
        <stop stopColor="#BF580F" offset={0.6431} />
        <stop stopColor="#B4530E" offset={0.7644} />
        <stop stopColor="#A34B0D" offset={0.8625} />
        <stop stopColor="#8D410B" offset={0.9456} />
        <stop stopColor="#7A380A" offset={1} />
      </linearGradient>
      <path
        d="m36.75 113.17c5.88-1.93 17.49-2.74 21.88 1.62 0.95 0.95 1.81 2.04 2.22 3.33 0.41 1.28 0.3 2.78-0.52 3.85-1.23 1.59-3.54 1.75-5.55 1.76-4.08 0.03-8.15 0.07-12.23 0.1-2.42 0.02-8.43 1.19-10.33-0.68-4.24-4.18 1.42-8.96 4.53-9.98z"
        fill="url(#h)"
        stroke="#99440B"
        strokeLinecap="round"
        strokeLinejoin="round"
        strokeMiterlimit={10}
        strokeWidth={2}
      />
      <linearGradient
        id="g"
        x1={56.704}
        x2={56.704}
        y1={132.81}
        y2={15.138}
        gradientUnits="userSpaceOnUse"
      >
        <stop stopColor="#C85C10" offset={0} />
        <stop stopColor="#C65B10" offset={0.4728} />
        <stop stopColor="#BF580F" offset={0.6431} />
        <stop stopColor="#B4530E" offset={0.7644} />
        <stop stopColor="#A34B0D" offset={0.8625} />
        <stop stopColor="#8D410B" offset={0.9456} />
        <stop stopColor="#7A380A" offset={1} />
      </linearGradient>
      <path
        d="m83.33 97.67c0-7.41-3.39-12.18-8.49-17.55-5.55-5.84-11.7-12.25-13.34-20.12-1.77-8.48-4.01-14.6-2.17-22.83-5.15-1.05-7.17 0.78-11.04 4.02-4.79 4.01-12.07 17.54-5.88 16.67 1.17-0.16-8.52 12.28-11.58 28.64-2.83 15.17 2.74 21.68 8.83 26.83 1.37 1.16 2.7 2.07 4.04 2.8-2.22 2.37-3.5 5.03-1.49 7.01 1.9 1.87 7.91 0.7 10.33 0.68 4.08-0.03 8.15-0.07 12.23-0.1 2.01-0.02 4.32-0.17 5.55-1.76 0.82-1.06 0.93-2.56 0.52-3.85-0.21-0.66-0.54-1.27-0.94-1.83 6.66-3.03 13.43-8.83 13.43-18.61z"
        fill="url(#g)"
        stroke="#99440B"
        strokeLinecap="round"
        strokeLinejoin="round"
        strokeMiterlimit={10}
        strokeWidth={2}
      />
      <linearGradient
        id="f"
        x1={40.122}
        x2={40.122}
        y1={117.32}
        y2={45.658}
        gradientTransform="matrix(.9631 -.2693 .2693 .9631 -21.625 21.531)"
        gradientUnits="userSpaceOnUse"
      >
        <stop stopColor="#C2945A" offset={0.001685} />
        <stop stopColor="#D7A669" offset={0.0966} />
        <stop stopColor="#E9B677" offset={0.217} />
        <stop stopColor="#F5C280" offset={0.3611} />
        <stop stopColor="#FDC885" offset={0.5506} />
        <stop stopColor="#FFCA87" offset={0.961} />
      </linearGradient>
      <path
        d="m43.79 114.94c0.35-0.69 0.87-1.28 1.58-1.59h0.01c0.37-0.16 0.73-0.28 1.1-0.36 0.13-0.1 0.27-0.2 0.41-0.29-13.74-23.75-3.83-50.57-4.88-47.59-5.68 3.06-14.26 19.73-10.51 32.89 2.67 9.38 7.97 14.35 12.26 16.97l0.03-0.03z"
        fill="url(#f)"
      />
      <path
        d="m26.92 105.19c-6.39 0-12.08-8.47-12.08-15.16 0-7.61 5.46-13.8 12.17-13.8s12.17 6.19 12.17 13.8c0 6.94-6.65 15.16-12.26 15.16z"
        fill="#C69C6D"
      />
      <path
        d="m27.01 77.24c6.16 0 11.17 5.74 11.17 12.8 0 6.35-6.21 14.16-11.26 14.16-5.38 0-11.08-7.28-11.08-14.16 0-7.06 5.01-12.8 11.17-12.8m0-2c-7.27 0-13.17 6.62-13.17 14.8 0 7.46 6.21 16.16 13.08 16.16 6.3 0 13.26-8.86 13.26-16.16 0-8.18-5.9-14.8-13.17-14.8z"
        fill="#99724D"
      />
      <path
        d="m13.43 85.46c-0.16 0-1.27-0.35-1.27-1.76 0-4.68 5.37-8.8 12.5-9.62 0.1-0.01 0.2-0.04 0.29-0.08 0.87-0.21 0.99-0.74 0.99-1.04 0-0.92 0.69-1.64 1.58-1.64 0.72 0 1.3 0.49 1.47 1.19-0.14 0.18-0.19 0.38-0.19 0.54-0.03 0.53 0.35 0.99 0.88 1.05 6.73 0.83 11.44 4.77 11.44 9.59 0 0.21 0.01 0.44 0.03 0.7 0.02 0.27 0.04 0.74 0.03 1.08h-27.75z"
        fill="#8C5219"
      />
      <path
        d="m27.53 72.32c0.17 0 0.29 0.07 0.37 0.17-0.07 0.2-0.09 0.38-0.09 0.51-0.06 1.05 0.71 1.97 1.75 2.1 6.22 0.76 10.56 4.3 10.56 8.61 0 0.21 0.01 0.46 0.03 0.74v0.02h-26.6c-0.12-0.05-0.38-0.22-0.38-0.78 0-4.08 5.1-7.86 11.61-8.61 0.17-0.02 0.33-0.06 0.48-0.12 1.61-0.39 1.7-1.73 1.7-2-0.02-0.32 0.18-0.64 0.57-0.64m0-2c-1.46 0-2.58 1.18-2.58 2.64 0 0.04-0.41 0.09-0.4 0.13-7.35 0.84-13.38 5.26-13.38 10.6v0.02c0 1.95 1.54 2.76 2.27 2.76h28.24c0.74 0 0.44-1.9 0.44-2.76v-0.02c0-5.29-5.05-9.69-12.32-10.58 0-0.05 0.25-0.1 0.25-0.15 0-1.46-1.07-2.64-2.52-2.64z"
        fill="#693411"
      />
      <linearGradient
        id="e"
        x1={72.267}
        x2={72.267}
        y1={112.51}
        y2={48.54}
        gradientTransform="matrix(.97 -.2431 .2431 .97 -19.869 19.058)"
        gradientUnits="userSpaceOnUse"
      >
        <stop stopColor="#C2945A" offset={0.001685} />
        <stop stopColor="#D7A669" offset={0.0966} />
        <stop stopColor="#E9B677" offset={0.217} />
        <stop stopColor="#F5C280" offset={0.3611} />
        <stop stopColor="#FDC885" offset={0.5506} />
        <stop stopColor="#FFCA87" offset={0.961} />
      </linearGradient>
      <path
        d="m60.36 73.17c2.4 4.68 5.73 8.81 8.74 13.13s5.77 8.96 6.79 14.12 0.38 9.79-3.27 13.58c5.37-3.37 9.11-8.07 9.88-14.38s-3.06-13.12-7.12-18c-2.77-3.33-5.9-6.55-8.7-9.86-2.06-2.43-4.74-7.74-5.63-10.8-0.43-1.47-0.01-5.12-2.61-4.61-2.39 0.47-1.24 6.08-1.04 7.69 0.39 3.2 1.5 6.28 2.96 9.13z"
        fill="url(#e)"
      />
      <linearGradient
        id="d"
        x1={20.314}
        x2={19.984}
        y1={4.7233}
        y2={38.941}
        gradientTransform="matrix(.9922 -.1243 .1243 .9922 -1.4885 3.2507)"
        gradientUnits="userSpaceOnUse"
      >
        <stop stopColor="#7A380A" offset={0} />
        <stop stopColor="#833C0B" offset={0.0364} />
        <stop stopColor="#9C480D" offset={0.1589} />
        <stop stopColor="#B0510E" offset={0.2959} />
        <stop stopColor="#BE570F" offset={0.4533} />
        <stop stopColor="#C65B10" offset={0.6489} />
        <stop stopColor="#C85C10" offset={1} />
      </linearGradient>
      <path
        d="m17.38 31.74c-0.55 0-1.01-0.23-1.43-0.73-3.73-4.35-3.76-11.09-3.43-18.8 0.07-1.53 1.35-2.44 2.56-2.44 0.54 0 1.06 0.17 1.5 0.51l12.91 9.56c-0.47 0.57-1.04 1.32-1.68 2.17-2.59 3.39-7.41 9.73-10.43 9.73z"
        fill="url(#d)"
      />
      <path
        d="m15.08 10.76c0.32 0 0.63 0.1 0.91 0.31l12.08 8.95c-0.32 0.42-0.68 0.88-1.05 1.37-2.13 2.8-7.11 9.35-9.63 9.35-0.17 0-0.37-0.03-0.67-0.38-3.49-4.07-3.51-10.61-3.19-18.11 0.03-0.93 0.81-1.49 1.55-1.49m0-2c-1.79 0-3.47 1.37-3.56 3.4-0.32 7.61-0.37 14.78 3.67 19.5 0.65 0.75 1.39 1.08 2.19 1.08 4.46 0 10.84-9.98 13.22-12.68 0.19-0.22 0.15-0.54-0.08-0.72l-13.35-9.87c-0.65-0.49-1.38-0.71-2.09-0.71z"
        fill="#A1480C"
      />
      <linearGradient
        id="c"
        x1={80.356}
        x2={80.032}
        y1={12.784}
        y2={46.287}
        gradientTransform="matrix(-.9922 -.1243 -.1243 .9922 140.33 3.2507)"
        gradientUnits="userSpaceOnUse"
      >
        <stop stopColor="#7A380A" offset={0} />
        <stop stopColor="#833C0B" offset={0.0364} />
        <stop stopColor="#9C480D" offset={0.1589} />
        <stop stopColor="#B0510E" offset={0.2959} />
        <stop stopColor="#BE570F" offset={0.4533} />
        <stop stopColor="#C65B10" offset={0.6489} />
        <stop stopColor="#C85C10" offset={1} />
      </linearGradient>
      <path
        d="m60.96 31.74c-3.02 0-7.84-6.34-10.43-9.75-0.66-0.87-1.25-1.64-1.72-2.21l12.25-9.01c0.53-0.39 1.14-0.6 1.77-0.6 1.43 0 2.95 1.09 3.02 2.91 0.27 6.99 0.22 13.62-3.46 17.92-0.42 0.51-0.88 0.74-1.43 0.74z"
        fill="url(#c)"
      />
      <path
        d="m62.84 11.18c0.95 0 1.97 0.73 2.02 1.95 0.27 6.79 0.22 13.21-3.23 17.23-0.3 0.35-0.51 0.38-0.67 0.38-2.52 0-7.5-6.55-9.63-9.35-0.38-0.5-0.73-0.96-1.05-1.37l11.39-8.44c0.36-0.26 0.75-0.4 1.17-0.4m0-2c-0.8 0-1.63 0.25-2.36 0.79l-12.27 9.09c-0.44 0.32-0.51 0.94-0.16 1.36 2.58 3.12 8.63 12.32 12.91 12.32 0.81 0 1.55-0.33 2.19-1.08 3.88-4.53 3.99-11.33 3.7-18.61-0.08-2.3-1.98-3.87-4.01-3.87z"
        fill="#A1480C"
      />
      <linearGradient
        id="b"
        x1={39.196}
        x2={39.196}
        y1={67.375}
        y2={14.683}
        gradientUnits="userSpaceOnUse"
      >
        <stop stopColor="#7A380A" offset={0} />
        <stop stopColor="#833C0B" offset={0.0364} />
        <stop stopColor="#9C480D" offset={0.1589} />
        <stop stopColor="#B0510E" offset={0.2959} />
        <stop stopColor="#BE570F" offset={0.4533} />
        <stop stopColor="#C65B10" offset={0.6489} />
        <stop stopColor="#C85C10" offset={1} />
      </linearGradient>
      <path
        d="m39.34 67.38c-17.07 0-28.11-8.88-28.11-22.61 0-14.79 10.51-30.08 28.11-30.08 10.34 0 16.83 5.16 20.45 9.49 4.68 5.59 7.36 13.21 7.36 20.89 0.01 14.17-10.13 22.31-27.81 22.31z"
        fill="url(#b)"
      />
      <path
        d="m39.34 15.68c9.95 0 16.2 4.96 19.68 9.13 4.53 5.42 7.13 12.8 7.13 20.24 0 6.92-2.39 12.28-7.1 15.92-4.58 3.53-11.39 5.4-19.71 5.4-7.73 0-14.61-1.98-19.39-5.56-5.05-3.79-7.72-9.34-7.72-16.05 0-7.39 2.65-14.71 7.26-20.08 3.54-4.1 9.85-9 19.85-9m0-2c-18.15 0-29.11 15.5-29.11 31.08s13.01 23.61 29.11 23.61c17.41 0 28.81-7.74 28.81-23.32s-10.65-31.37-28.81-31.37z"
        fill="#99440B"
      />
      <linearGradient
        id="a"
        x1={39.21}
        x2={39.21}
        y1={20.428}
        y2={65.882}
        gradientUnits="userSpaceOnUse"
      >
        <stop stopColor="#FFCA87" offset={0.039} />
        <stop stopColor="#FDC885" offset={0.4494} />
        <stop stopColor="#F5C280" offset={0.6389} />
        <stop stopColor="#E9B677" offset={0.783} />
        <stop stopColor="#D7A669" offset={0.9034} />
        <stop stopColor="#C2945A" offset={0.9983} />
      </linearGradient>
      <path
        d="M65.09,50.27c-0.79-2.59-5.1-3.86-7.28-4.75c-3.35-1.36-7.05-2.44-10.05-4.5 c-2.84-1.96-2.21-4.8-0.97-7.73c1.35-3.18,1.77-6.29-0.14-9.35c-1.57-2.51-4.32-3.82-7.24-3.82c-5.98,0-9.92,5.76-8.05,11.39 c0.4,1.2,1.15,2.28,1.53,3.47c0.3,0.97,0.35,2.18,0.35,3.19c0,4.89-18.5,6.61-19.89,11.16C12.21,53,21,66,39.4,66 S66.21,53.94,65.09,50.27z"
        fill="url(#a)"
      />
      <path
        d="m42.72 54.03-3.41-2.49-3.2 2.49c-0.22 0.2-0.11 0.48-0.11 0.77v0.13c0 2.02 0.08 6.08 2.1 6.08h2.31c2.02 0 2.59-4.06 2.59-6.08v-0.06c0-0.3-0.07-0.64-0.28-0.84z"
        fill="#fff"
      />
      <g
        fill="none"
        stroke="#A8855D"
        strokeLinecap="round"
        strokeLinejoin="round"
        strokeMiterlimit={10}
        strokeWidth={2}
      >
        <path d="m39.2 49.23c0 2.44-1.98 4.42-4.42 4.42s-4.42-1.98-4.42-4.42" />
        <path d="m48.32 49.23c0 2.44-1.98 4.42-4.42 4.42s-4.42-1.98-4.42-4.42" />
      </g>
      <linearGradient
        id="k"
        x1={39.344}
        x2={39.344}
        y1={41.196}
        y2={48.18}
        gradientUnits="userSpaceOnUse"
      >
        <stop stopColor="#575757" offset={0.1628} />
        <stop stopColor="#262626" offset={0.6453} />
        <stop offset={1} />
      </linearGradient>
      <ellipse cx={39.34} cy={44.68} rx={6.09} ry={3.8} fill="url(#k)" />
      <path d="m55.91 37.18-3.73-0.86 3.83-1.81c0.04-0.02 0.09-0.05 0.13-0.08 0.8-0.56 0.93-1.72 0.21-2.44-0.56-0.56-1.46-0.57-2.11-0.12-1.52 1.06-5.09 3.57-6.49 4.55-0.25 0.18-0.21 0.56 0.07 0.68l7.04 3.08 0.15 0.06c0.93 0.3 1.94-0.3 2.07-1.32 0.1-0.77-0.43-1.5-1.17-1.74z" />
      <path d="m23.05 34.52c-0.04-0.02-0.09-0.05-0.13-0.08-0.8-0.56-0.93-1.72-0.21-2.44 0.56-0.56 1.46-0.57 2.11-0.12 1.52 1.06 5.09 3.57 6.49 4.55 0.25 0.18 0.21 0.56-0.07 0.68l-7.04 3.08-0.15 0.06c-0.93 0.3-1.94-0.3-2.07-1.32-0.1-0.77 0.42-1.5 1.17-1.74l3.73-0.86" />
      <linearGradient
        id="j"
        x1={41.196}
        x2={41.196}
        y1={70.755}
        y2={93.382}
        gradientUnits="userSpaceOnUse"
      >
        <stop stopColor="#C85C10" offset={0} />
        <stop stopColor="#C65B10" offset={0.4728} />
        <stop stopColor="#BF580F" offset={0.6431} />
        <stop stopColor="#B4530E" offset={0.7644} />
        <stop stopColor="#A34B0D" offset={0.8625} />
        <stop stopColor="#8D410B" offset={0.9456} />
        <stop stopColor="#7A380A" offset={1} />
      </linearGradient>
      <path
        d="m49.9 67.34c-10.13 0.96-19.49 7.84-23.41 17.23-0.47 1.11-0.86 2.28-0.84 3.49s0.54 2.46 1.56 3.1c1.11 0.7 2.6 0.54 3.78-0.04s2.14-1.52 3.13-2.38c6.22-5.47 14.37-8.69 22.65-8.96"
        fill="url(#j)"
        stroke="#99440B"
        strokeLinecap="round"
        strokeLinejoin="round"
        strokeMiterlimit={10}
        strokeWidth={2}
      />
      <path
        d="m42.58 117.58c3.5-4.92 7.25-5.92 15.25-4.25"
        fill="none"
        stroke="#99440B"
        strokeLinecap="round"
        strokeLinejoin="round"
        strokeMiterlimit={10}
        strokeWidth={2}
      />
    </svg>
  );
}

export default Chipmunk;
