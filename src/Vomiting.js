"use strict";

Object.defineProperty(exports, "__esModule", {
  value: true
});

var _react = require("react");

var _react2 = _interopRequireDefault(_react);

function _interopRequireDefault(obj) {
  return obj && obj.__esModule ? obj : { default: obj };
}

function _extends() {
  _extends =
    Object.assign ||
    function(target) {
      for (var i = 1; i < arguments.length; i++) {
        var source = arguments[i];
        for (var key in source) {
          if (Object.prototype.hasOwnProperty.call(source, key)) {
            target[key] = source[key];
          }
        }
      }
      return target;
    };
  return _extends.apply(this, arguments);
}

function Vomiting(props) {
  return _react2.default.createElement(
    "svg",
    _extends(
      {
        enableBackground: "new 0 0 128 128",
        viewBox: "0 0 128 128"
      },
      props
    ),
    _react2.default.createElement(
      "symbol",
      {
        viewBox: "-64.5 -64.5 129 129"
      },
      _react2.default.createElement(
        "g",
        {
          fill: "none",
          stroke: "#4285F4",
          strokeMiterlimit: 10
        },
        _react2.default.createElement("rect", {
          x: -64,
          y: -64,
          width: 128,
          height: 128
        }),
        _react2.default.createElement("path", {
          d:
            "M36.95,37.82C27.32,46.32,14.2,51,0,51 c-14.27,0-27.39-4.62-36.96-13.01C-47.45,28.79-53,15.65-53,0c0-15.58,5.55-28.69,16.04-37.92C-27.36-46.35-14.24-51,0-51 c14.17,0,27.29,4.71,36.95,13.25C47.45-28.45,53-15.4,53,0C53,15.47,47.45,28.55,36.95,37.82z"
        }),
        _react2.default.createElement("path", {
          d:
            "m0 55c-29.59 0-57-19.01-57-55 0-35.8 27.41-55 57-55s57 19.69 57 55c0 35.51-27.41 55-57 55z"
        }),
        _react2.default.createElement("path", {
          d:
            "M0-43c-12.29,0-23.54,3.94-31.68,11.09 C-40.39-24.25-45-13.21-45,0c0,29.7,22.6,43,45,43c21.67,0,45-13.46,45-43S21.67-43,0-43L0-43z"
        }),
        _react2.default.createElement("line", {
          x1: -0.01,
          x2: -0.01,
          y1: 51,
          y2: -51
        }),
        _react2.default.createElement("line", {
          x1: -16,
          x2: -16,
          y1: 48.95,
          y2: -48.93
        }),
        _react2.default.createElement("line", {
          x1: 15.99,
          x2: 15.99,
          y1: 48.91,
          y2: -48.93
        }),
        _react2.default.createElement("line", {
          x1: 53,
          x2: -53,
          y1: 0.08,
          y2: 0.08
        })
      )
    ),
    _react2.default.createElement("path", {
      d:
        "m4.15 64c0 37.28 28.78 57.75 59.85 57.75s59.85-19.96 59.85-57.75c0-37.59-28.78-57.75-59.85-57.75s-59.85 20.67-59.85 57.75z",
      fill: "#FEE133"
    }),
    _react2.default.createElement(
      "linearGradient",
      {
        id: "g",
        x1: -10532,
        x2: -10532,
        y1: -3235.8,
        y2: -3144.8,
        gradientTransform: "matrix(-1 0 0 -1 -10318 -3126)",
        gradientUnits: "userSpaceOnUse"
      },
      _react2.default.createElement("stop", {
        stopColor: "#40C0E7",
        stopOpacity: 0,
        offset: 0.3853
      }),
      _react2.default.createElement("stop", {
        stopColor: "#40C0E7",
        offset: 1
      })
    ),
    _react2.default.createElement("path", {
      d:
        "m154.15 64c0 37.28 28.78 57.75 59.85 57.75s59.85-19.96 59.85-57.75c0-37.59-28.78-57.75-59.85-57.75s-59.85 20.67-59.85 57.75z",
      fill: "url(#g)"
    }),
    _react2.default.createElement("path", {
      d:
        "m64 10.25c15 0 28.83 4.9 38.94 13.78 11.06 9.73 16.91 23.55 16.91 39.97 0 16.49-5.85 30.34-16.91 40.04-10.08 8.84-23.91 13.71-38.94 13.71-14.95 0-28.78-4.93-38.93-13.89-11.07-9.77-16.92-23.55-16.92-39.86 0-16.23 5.85-29.99 16.92-39.78 10.18-9.01 24.01-13.97 38.93-13.97m0-4c-31.07 0-59.85 20.67-59.85 57.75 0 37.28 28.78 57.75 59.85 57.75s59.85-19.96 59.85-57.75c0-37.59-28.78-57.75-59.85-57.75z",
      fill: "#47A9B0"
    }),
    _react2.default.createElement(
      "linearGradient",
      {
        id: "f",
        x1: -10382,
        x2: -10382,
        y1: -3234.2,
        y2: -3146.4,
        gradientTransform: "matrix(-1 0 0 -1 -10318 -3126)",
        gradientUnits: "userSpaceOnUse"
      },
      _react2.default.createElement("stop", {
        stopColor: "#FEE133",
        offset: 0.05
      }),
      _react2.default.createElement("stop", {
        stopColor: "#D5DA5A",
        offset: 0.1556
      }),
      _react2.default.createElement("stop", {
        stopColor: "#A8D285",
        offset: 0.2902
      }),
      _react2.default.createElement("stop", {
        stopColor: "#82CCA8",
        offset: 0.4258
      }),
      _react2.default.createElement("stop", {
        stopColor: "#65C6C4",
        offset: 0.5602
      }),
      _react2.default.createElement("stop", {
        stopColor: "#51C3D7",
        offset: 0.6931
      }),
      _react2.default.createElement("stop", {
        stopColor: "#44C1E3",
        offset: 0.8239
      }),
      _react2.default.createElement("stop", {
        stopColor: "#40C0E7",
        offset: 0.95
      })
    ),
    _react2.default.createElement("path", {
      d:
        "m64 119.75c-15.44 0-29.74-5.11-40.25-14.39-11.51-10.16-17.6-24.46-17.6-41.36 0-16.82 6.08-31.09 17.6-41.28 10.54-9.33 24.84-14.47 40.25-14.47 27.86 0 57.85 17.45 57.85 55.75s-29.99 55.75-57.85 55.75z",
      fill: "url(#f)"
    }),
    _react2.default.createElement("path", {
      d:
        "m64 10.25c15 0 28.83 4.9 38.94 13.78 11.06 9.73 16.91 23.55 16.91 39.97 0 16.49-5.85 30.34-16.91 40.04-10.08 8.84-23.91 13.71-38.94 13.71-14.95 0-28.78-4.93-38.93-13.89-11.07-9.77-16.92-23.55-16.92-39.86 0-16.23 5.85-29.99 16.92-39.78 10.18-9.01 24.01-13.97 38.93-13.97m0-4c-31.07 0-59.85 20.67-59.85 57.75 0 37.28 28.78 57.75 59.85 57.75s59.85-19.96 59.85-57.75c0-37.59-28.78-57.75-59.85-57.75z",
      fill: "#47A9B0"
    }),
    _react2.default.createElement("path", {
      d:
        "m51.1 32.47s-0.01 0.28-0.05 0.77c-0.02 0.13-0.01 0.24-0.05 0.42-0.03 0.18-0.09 0.37-0.14 0.58s-0.11 0.43-0.17 0.67c-0.07 0.22-0.16 0.43-0.24 0.66-0.16 0.47-0.39 0.95-0.66 1.44-0.26 0.5-0.59 0.99-0.95 1.49-0.39 0.47-0.77 0.99-1.24 1.42-0.45 0.47-0.96 0.87-1.47 1.29-0.55 0.36-1.08 0.74-1.65 1.04-0.55 0.33-1.16 0.55-1.72 0.79-0.58 0.2-1.15 0.38-1.7 0.49-0.55 0.12-1.08 0.2-1.57 0.23-0.49 0.06-0.94 0.05-1.34 0.02-0.4-0.01-0.74-0.05-1.03-0.1-0.57-0.1-0.88-0.18-0.88-0.18-0.95-0.24-1.54-1.2-1.3-2.15 0.15-0.62 0.62-1.09 1.19-1.27l0.3-0.09s0.24-0.07 0.64-0.17c0.2-0.04 0.44-0.12 0.71-0.2 0.27-0.07 0.57-0.17 0.89-0.29 0.32-0.1 0.66-0.26 1.02-0.4 0.36-0.13 0.71-0.35 1.09-0.52 0.37-0.21 0.75-0.41 1.12-0.65 0.39-0.21 0.73-0.5 1.1-0.75 0.35-0.28 0.72-0.53 1.03-0.84 0.33-0.29 0.64-0.59 0.94-0.88 0.26-0.32 0.58-0.6 0.81-0.9 0.24-0.3 0.48-0.58 0.67-0.86 0.21-0.29 0.38-0.52 0.51-0.72 0.07-0.1 0.14-0.2 0.21-0.29 0.07-0.1 0.13-0.24 0.19-0.34 0.23-0.43 0.38-0.68 0.38-0.68 0.52-0.86 1.64-1.14 2.5-0.62 0.56 0.33 0.88 0.96 0.86 1.59z",
      fill: "#6D4C41"
    }),
    _react2.default.createElement("path", {
      d:
        "m80.28 31.48s0.15 0.24 0.38 0.68c0.06 0.1 0.12 0.25 0.19 0.34s0.14 0.19 0.21 0.29c0.13 0.2 0.3 0.43 0.51 0.72 0.19 0.28 0.43 0.56 0.67 0.86 0.23 0.31 0.54 0.58 0.81 0.9 0.3 0.29 0.61 0.6 0.94 0.88 0.31 0.31 0.68 0.56 1.03 0.84 0.37 0.25 0.72 0.54 1.1 0.75 0.37 0.24 0.75 0.45 1.12 0.65 0.38 0.16 0.74 0.38 1.09 0.52 0.36 0.14 0.69 0.3 1.02 0.4 0.32 0.12 0.62 0.22 0.89 0.29 0.27 0.08 0.51 0.16 0.71 0.2 0.4 0.1 0.64 0.17 0.64 0.17l0.29 0.09c0.94 0.29 1.46 1.29 1.16 2.23-0.19 0.61-0.69 1.05-1.27 1.19 0 0-0.31 0.08-0.88 0.18-0.28 0.06-0.63 0.09-1.03 0.1-0.4 0.03-0.85 0.04-1.34-0.02-0.5-0.02-1.02-0.1-1.57-0.23-0.56-0.1-1.12-0.29-1.7-0.49-0.56-0.24-1.17-0.45-1.72-0.79-0.57-0.3-1.11-0.68-1.65-1.04-0.5-0.42-1.02-0.82-1.47-1.29-0.48-0.43-0.85-0.95-1.24-1.42-0.36-0.5-0.7-0.99-0.95-1.49-0.27-0.49-0.5-0.97-0.66-1.44-0.09-0.23-0.18-0.44-0.24-0.66-0.06-0.23-0.12-0.46-0.17-0.67s-0.1-0.4-0.14-0.58c-0.03-0.17-0.03-0.29-0.05-0.42-0.04-0.49-0.05-0.77-0.05-0.77-0.03-1.01 0.77-1.84 1.77-1.87 0.67 0 1.27 0.36 1.6 0.9z",
      fill: "#6D4C41"
    }),
    _react2.default.createElement(
      "defs",
      null,
      _react2.default.createElement("path", {
        id: "d",
        d:
          "M64,68.82c-16.95,0-35.33,14.17-35.33,31.67c0,4.89,5.61,7.73,10.62,5.75 c6.19-2.46,15.32-4.92,24.71-4.92s18.52,2.47,24.71,4.92c5.01,1.99,10.62-0.86,10.62-5.75C99.33,83,79.66,68.82,64,68.82z"
      })
    ),
    _react2.default.createElement(
      "clipPath",
      {
        id: "a"
      },
      _react2.default.createElement("use", {
        xlinkHref: "#d"
      })
    ),
    _react2.default.createElement("path", {
      d:
        "m64 68.82c-16.95 0-35.33 14.17-35.33 31.67 0 4.89 5.61 7.73 10.62 5.75 6.19-2.46 15.32-4.92 24.71-4.92s18.52 2.47 24.71 4.92c5.01 1.99 10.62-0.86 10.62-5.75 0-17.49-19.67-31.67-35.33-31.67z",
      clipPath: "url(#a)",
      fill: "#242424"
    }),
    _react2.default.createElement("path", {
      d:
        "m64 81.02c12.42 0 23.94 1.61 33.04 4.56v-28.62h-66.08v28.61c9.1-2.94 20.62-4.55 33.04-4.55z",
      clipPath: "url(#a)",
      fill: "#fff"
    }),
    _react2.default.createElement(
      "g",
      {
        clipPath: "url(#a)"
      },
      _react2.default.createElement(
        "defs",
        null,
        _react2.default.createElement("path", {
          id: "c",
          d:
            "m64 81.02c12.42 0 23.94 1.61 33.04 4.56v-28.62h-66.08v28.61c9.1-2.94 20.62-4.55 33.04-4.55z"
        })
      ),
      _react2.default.createElement(
        "clipPath",
        {
          id: "b"
        },
        _react2.default.createElement("use", {
          xlinkHref: "#c"
        })
      ),
      _react2.default.createElement(
        "g",
        {
          clipPath: "url(#b)"
        },
        _react2.default.createElement("path", {
          d:
            "m64 72.79c6.43 0 14.15 3.11 20.15 8.1 6.71 5.59 10.4 12.55 10.4 19.6 0 1.66-1.31 2.42-2.52 2.42-0.41 0-0.83-0.08-1.25-0.25-4.02-1.6-14.69-5.31-26.78-5.31s-22.75 3.71-26.78 5.31c-0.42 0.17-0.84 0.25-1.25 0.25-1.21 0-2.52-0.76-2.52-2.42 0-15.6 16.42-27.7 30.55-27.7m0-3.97c-16.95 0-35.33 14.17-35.33 31.67 0 3.82 3.42 6.39 7.3 6.39 1.09 0 2.22-0.2 3.32-0.64 6.19-2.46 15.32-4.92 24.71-4.92s18.52 2.47 24.71 4.92c1.1 0.44 2.23 0.64 3.32 0.64 3.88 0 7.3-2.57 7.3-6.39 0-17.49-19.67-31.67-35.33-31.67z",
          fill: "#47A9B0"
        })
      )
    ),
    _react2.default.createElement("ellipse", {
      transform: "matrix(.9891 -.1473 .1473 .9891 -15.391 17.45)",
      cx: 110.1,
      cy: 112.62,
      rx: 2.65,
      ry: 1.56,
      fill: "#368231"
    }),
    _react2.default.createElement("ellipse", {
      transform: "matrix(.2971 -.9548 .9548 .2971 -90.316 99.056)",
      cx: 22.12,
      cy: 110.88,
      rx: 2.16,
      ry: 3.67,
      fill: "#368231"
    }),
    _react2.default.createElement(
      "linearGradient",
      {
        id: "e",
        x1: 64.981,
        x2: 64.981,
        y1: 85.333,
        y2: 125.34,
        gradientUnits: "userSpaceOnUse"
      },
      _react2.default.createElement("stop", {
        stopColor: "#469643",
        offset: 0
      }),
      _react2.default.createElement("stop", {
        stopColor: "#489743",
        offset: 0.0203
      }),
      _react2.default.createElement("stop", {
        stopColor: "#6DB141",
        offset: 0.4427
      }),
      _react2.default.createElement("stop", {
        stopColor: "#84C03F",
        offset: 0.7841
      }),
      _react2.default.createElement("stop", {
        stopColor: "#8CC63F",
        offset: 1
      })
    ),
    _react2.default.createElement("path", {
      d:
        "m96.7 119.17c4.87-1.47 6.65-3.17 6.42-4.39-0.3-1.58-7.12-1.82-15.12-0.68v-26.6s-8.11-2.42-24.5-2.42c-18.19 0-24.5 2.42-24.5 2.42v23.86c-5-0.29-8.79 0.33-8.97 1.68-0.14 1.07 3.03 2.36 6.92 3.46-7.59 0.78-12.14 2-12.14 3.37 0 2.35 14.45 4.25 32.05 4.25 7.87 0 15.16-0.38 20.72-1.01 2.91 0.47 7.02 0.76 11.53 0.76 8.84 0 16.02-1.12 16.02-2.5 0.01-0.95-3.4-1.78-8.43-2.2z",
      fill: "url(#e)"
    }),
    _react2.default.createElement("ellipse", {
      cx: 58.25,
      cy: 110.54,
      rx: 2.42,
      ry: 4.71,
      fill: "#368231"
    }),
    _react2.default.createElement("ellipse", {
      cx: 62.75,
      cy: 93.92,
      rx: 2.25,
      ry: 4.08,
      fill: "#368231"
    }),
    _react2.default.createElement("ellipse", {
      cx: 76.38,
      cy: 103.83,
      rx: 3.96,
      ry: 7.25,
      fill: "#368231"
    }),
    _react2.default.createElement("ellipse", {
      cx: 48.21,
      cy: 98.31,
      rx: 1.88,
      ry: 3.27,
      fill: "#368231"
    }),
    _react2.default.createElement("path", {
      d:
        "m102.34 118.42c2.3-1.43 2.71-2.84 2.5-3.91-0.28-1.48-2.62-3.65-14.84-2.12l0.02-24.39-2.72-0.67-0.3 28.5 1.71-0.24c7.22-1.03 11.38-0.73 12.86-0.33-0.6 0.55-2.12 1.5-5.32 2.47l-7.53 2.28 7.84 0.65c2.38 0.2 4.07 0.46 5.22 0.71-2.38 0.5-6.65 1-12.66 1-4.25 0-8.36-0.27-11.29-0.74l-0.2-0.03-0.21 0.02c-5.7 0.65-13 1-20.54 1-15.84 0-26.59-1.47-29.8-2.75 1.37-0.54 4.42-1.29 10.14-1.88l7.79-0.8-7.53-2.14c-2.58-0.73-4.14-1.37-5-1.81 1.17-0.31 3.44-0.59 6.92-0.39l1.59 0.09 0.09-26.04-3.08 0.79v22.11c-4.32-0.11-8.86 0.42-9.21 3.05-0.13 1 0.28 1.95 2.13 2.97-6.5 1.15-7.55 2.62-7.55 4.06 0 1.84 1.82 3.31 10.71 4.49 6.12 0.81 14.22 1.26 22.81 1.26 7.56 0 14.89-0.35 20.67-1 3.1 0.48 7.19 0.75 11.56 0.75 14.48 0 17.52-2.17 17.52-4 0.01-1.09-0.62-2.18-4.3-2.96z",
      fill: "#368231"
    }),
    _react2.default.createElement("path", {
      d: "m103.91 122h0.01-0.01z",
      fill: "#368231"
    }),
    _react2.default.createElement("path", {
      d:
        "m33.19 63.55 12.17-2.36-12.55-5.21c-0.14-0.07-0.28-0.15-0.42-0.23-2.61-1.6-3.06-4.89-0.72-6.93 1.81-1.58 4.76-1.6 6.88-0.3 4.98 3.05 16.7 10.24 21.27 13.04 0.82 0.5 0.7 1.58-0.22 1.93l-22.91 8.6c-0.15 0.06-0.32 0.11-0.48 0.16-3.03 0.84-6.32-0.89-6.79-3.78-0.35-2.2 1.35-4.25 3.77-4.92z",
      fill: "#242424"
    }),
    _react2.default.createElement("path", {
      d:
        "m95.81 63.3-12.17-2.36 12.55-5.21c0.14-0.07 0.28-0.15 0.42-0.23 2.61-1.6 3.06-4.89 0.72-6.93-1.81-1.58-4.76-1.6-6.88-0.3-4.98 3.05-16.7 10.24-21.27 13.04-0.82 0.5-0.7 1.58 0.22 1.93l22.91 8.6c0.15 0.06 0.32 0.11 0.48 0.16 3.03 0.84 6.32-0.89 6.79-3.78 0.35-2.2-1.35-4.25-3.77-4.92z",
      fill: "#242424"
    })
  );
}

exports.default = Vomiting;
