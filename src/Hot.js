"use strict";

Object.defineProperty(exports, "__esModule", {
  value: true
});

var _react = require("react");

var _react2 = _interopRequireDefault(_react);

function _interopRequireDefault(obj) {
  return obj && obj.__esModule ? obj : { default: obj };
}

function _extends() {
  _extends =
    Object.assign ||
    function(target) {
      for (var i = 1; i < arguments.length; i++) {
        var source = arguments[i];
        for (var key in source) {
          if (Object.prototype.hasOwnProperty.call(source, key)) {
            target[key] = source[key];
          }
        }
      }
      return target;
    };
  return _extends.apply(this, arguments);
}

function Hot(props) {
  return _react2.default.createElement(
    "svg",
    _extends(
      {
        enableBackground: "new 0 0 128 128",
        viewBox: "0 0 128 128"
      },
      props
    ),
    _react2.default.createElement(
      "symbol",
      {
        viewBox: "-64.5 -64.5 129 129"
      },
      _react2.default.createElement(
        "g",
        {
          fill: "none",
          stroke: "#4285F4",
          strokeMiterlimit: 10
        },
        _react2.default.createElement("rect", {
          x: -64,
          y: -64,
          width: 128,
          height: 128
        }),
        _react2.default.createElement("path", {
          d:
            "M36.95,37.82C27.32,46.32,14.2,51,0,51 c-14.27,0-27.39-4.62-36.96-13.01C-47.45,28.79-53,15.65-53,0c0-15.58,5.55-28.69,16.04-37.92C-27.36-46.35-14.24-51,0-51 c14.17,0,27.29,4.71,36.95,13.25C47.45-28.45,53-15.4,53,0C53,15.47,47.45,28.55,36.95,37.82z"
        }),
        _react2.default.createElement("path", {
          d:
            "m0 55c-29.59 0-57-19.01-57-55 0-35.8 27.41-55 57-55s57 19.69 57 55c0 35.51-27.41 55-57 55z"
        }),
        _react2.default.createElement("path", {
          d:
            "M0-43c-12.29,0-23.54,3.94-31.68,11.09 C-40.39-24.25-45-13.21-45,0c0,29.7,22.6,43,45,43c21.67,0,45-13.46,45-43S21.67-43,0-43L0-43z"
        }),
        _react2.default.createElement("line", {
          x1: -0.01,
          x2: -0.01,
          y1: 51,
          y2: -51
        }),
        _react2.default.createElement("line", {
          x1: -16,
          x2: -16,
          y1: 48.95,
          y2: -48.93
        }),
        _react2.default.createElement("line", {
          x1: 15.99,
          x2: 15.99,
          y1: 48.91,
          y2: -48.93
        }),
        _react2.default.createElement("line", {
          x1: 53,
          x2: -53,
          y1: 0.08,
          y2: 0.08
        })
      )
    ),
    _react2.default.createElement(
      "linearGradient",
      {
        id: "e",
        x1: 63.996,
        x2: 63.996,
        y1: 19.838,
        y2: 107.58,
        gradientUnits: "userSpaceOnUse"
      },
      _react2.default.createElement("stop", {
        stopColor: "#FF5100",
        offset: 0
      }),
      _react2.default.createElement("stop", {
        stopColor: "#FE5D07",
        offset: 0.1439
      }),
      _react2.default.createElement("stop", {
        stopColor: "#FD7E19",
        offset: 0.4068
      }),
      _react2.default.createElement("stop", {
        stopColor: "#FBB337",
        offset: 0.7568
      }),
      _react2.default.createElement("stop", {
        stopColor: "#FAC742",
        offset: 0.8772
      })
    ),
    _react2.default.createElement("path", {
      d:
        "m64 119.74c-27.86 0-57.84-17.44-57.84-55.74s29.98-55.74 57.84-55.74c15.44 0 29.73 5.11 40.25 14.39 11.51 10.16 17.59 24.46 17.59 41.35 0 16.82-6.08 31.09-17.59 41.27-10.55 9.33-24.84 14.47-40.25 14.47z",
      fill: "url(#e)"
    }),
    _react2.default.createElement("path", {
      d:
        "m64 10.26c14.95 0 28.77 4.93 38.92 13.89 11.07 9.77 16.92 23.55 16.92 39.85 0 16.23-5.85 29.98-16.92 39.78-10.18 9.01-24 13.96-38.92 13.96-15 0-28.82-4.89-38.93-13.78-11.07-9.73-16.91-23.54-16.91-39.96 0-16.49 5.85-30.34 16.91-40.03 10.07-8.84 23.9-13.71 38.93-13.71m0-4c-31.07 0-59.84 19.95-59.84 57.74 0 37.58 28.77 57.74 59.84 57.74s59.84-20.67 59.84-57.74c0-37.27-28.78-57.74-59.84-57.74z",
      fill: "#E75900"
    }),
    _react2.default.createElement(
      "g",
      {
        fill: "#242424"
      },
      _react2.default.createElement("path", {
        d:
          "m36.47 49.39c-0.3 0.11-0.61 0.18-0.92 0.22-0.03 0.36-0.05 0.74-0.05 1.12 0 6.08 3.95 9.75 8.29 9.75s8.29-3.67 8.29-9.75c0-2.85-0.87-5.16-2.24-6.83-4.49 1.78-8.86 3.8-13.37 5.49z"
      }),
      _react2.default.createElement("path", {
        d:
          "m91.53 49.39c0.3 0.11 0.61 0.18 0.92 0.22 0.03 0.36 0.05 0.74 0.05 1.12 0 6.08-3.95 9.75-8.29 9.75s-8.29-3.67-8.29-9.75c0-2.85 0.87-5.16 2.24-6.83 4.49 1.78 8.86 3.8 13.37 5.49z"
      })
    ),
    _react2.default.createElement("path", {
      d: "m103.91 122h0.01-0.01z",
      fill: "#368231"
    }),
    _react2.default.createElement(
      "g",
      {
        opacity: 0.5
      },
      _react2.default.createElement("path", {
        d:
          "m51.1 23.55s-0.01 0.28-0.05 0.77c-0.02 0.13-0.01 0.24-0.05 0.42-0.03 0.18-0.09 0.37-0.14 0.58s-0.11 0.43-0.17 0.67c-0.07 0.22-0.16 0.43-0.24 0.66-0.16 0.47-0.39 0.95-0.66 1.44-0.26 0.5-0.59 0.99-0.95 1.49-0.39 0.47-0.77 0.99-1.24 1.42-0.45 0.47-0.96 0.87-1.47 1.29-0.55 0.36-1.08 0.74-1.65 1.04-0.55 0.33-1.16 0.55-1.72 0.79-0.58 0.2-1.15 0.38-1.7 0.49-0.55 0.12-1.08 0.2-1.57 0.23-0.49 0.06-0.94 0.05-1.34 0.02-0.4-0.01-0.74-0.05-1.03-0.1-0.57-0.1-0.88-0.18-0.88-0.18-0.95-0.24-1.54-1.2-1.3-2.15 0.15-0.62 0.62-1.09 1.19-1.27l0.3-0.09s0.24-0.07 0.64-0.17c0.2-0.04 0.44-0.12 0.71-0.2 0.27-0.07 0.57-0.17 0.89-0.29 0.32-0.1 0.66-0.26 1.02-0.4 0.36-0.13 0.71-0.35 1.09-0.52 0.37-0.21 0.75-0.41 1.12-0.65 0.39-0.21 0.73-0.5 1.1-0.75 0.35-0.28 0.72-0.53 1.03-0.84 0.33-0.29 0.64-0.59 0.94-0.88 0.26-0.32 0.58-0.6 0.81-0.9 0.24-0.3 0.48-0.58 0.67-0.86 0.21-0.29 0.38-0.52 0.51-0.72 0.07-0.1 0.14-0.2 0.21-0.29 0.07-0.1 0.13-0.24 0.19-0.34 0.23-0.43 0.38-0.68 0.38-0.68 0.52-0.86 1.64-1.14 2.5-0.62 0.56 0.33 0.88 0.96 0.86 1.59z",
        fill: "#6D4C41",
        stroke: "#6D4C41",
        strokeMiterlimit: 10
      })
    ),
    _react2.default.createElement(
      "g",
      {
        opacity: 0.5
      },
      _react2.default.createElement("path", {
        d:
          "m80.28 22.56s0.15 0.24 0.38 0.68c0.06 0.1 0.12 0.25 0.19 0.34s0.14 0.19 0.21 0.29c0.13 0.2 0.3 0.43 0.51 0.72 0.19 0.28 0.43 0.56 0.67 0.86 0.23 0.31 0.54 0.58 0.81 0.9 0.3 0.29 0.61 0.6 0.94 0.88 0.31 0.31 0.68 0.56 1.03 0.84 0.37 0.25 0.72 0.54 1.1 0.75 0.37 0.24 0.75 0.45 1.12 0.65 0.38 0.16 0.74 0.38 1.09 0.52 0.36 0.14 0.69 0.3 1.02 0.4 0.32 0.12 0.62 0.22 0.89 0.29 0.27 0.08 0.51 0.16 0.71 0.2 0.4 0.1 0.64 0.17 0.64 0.17l0.29 0.09c0.94 0.29 1.46 1.29 1.16 2.23-0.19 0.61-0.69 1.05-1.27 1.19 0 0-0.31 0.08-0.88 0.18-0.28 0.06-0.63 0.09-1.03 0.1-0.4 0.03-0.85 0.04-1.34-0.02-0.5-0.02-1.02-0.1-1.57-0.23-0.56-0.1-1.12-0.29-1.7-0.49-0.56-0.24-1.17-0.45-1.72-0.79-0.57-0.3-1.11-0.68-1.65-1.04-0.5-0.42-1.02-0.82-1.47-1.29-0.48-0.43-0.85-0.95-1.24-1.42-0.36-0.5-0.7-0.99-0.95-1.49-0.27-0.49-0.5-0.97-0.66-1.44-0.09-0.23-0.18-0.44-0.24-0.66-0.06-0.23-0.12-0.46-0.17-0.67s-0.1-0.4-0.14-0.58c-0.03-0.17-0.03-0.29-0.05-0.42-0.04-0.49-0.05-0.77-0.05-0.77-0.03-1.01 0.77-1.84 1.77-1.87 0.67 0 1.27 0.36 1.6 0.9z",
        fill: "#6D4C41",
        stroke: "#6D4C41",
        strokeMiterlimit: 10
      })
    ),
    _react2.default.createElement("ellipse", {
      cx: 64,
      cy: 89.5,
      rx: 37,
      ry: 13.5,
      fill: "#242424"
    }),
    _react2.default.createElement(
      "defs",
      null,
      _react2.default.createElement("path", {
        id: "d",
        d: "M29.98,86.35v44.16h68.05V90c-15.31-10-54.79-10.48-67.27,2"
      })
    ),
    _react2.default.createElement(
      "clipPath",
      {
        id: "a"
      },
      _react2.default.createElement("use", {
        xlinkHref: "#d"
      })
    ),
    _react2.default.createElement("path", {
      d:
        "m66.74 118.16h-3.28c-9.51 0-17.21-7.71-17.21-17.21v-38.77h37.71v38.76c0 9.51-7.71 17.22-17.22 17.22z",
      clipPath: "url(#a)",
      fill: "#f04"
    }),
    _react2.default.createElement(
      "g",
      {
        clipPath: "url(#a)",
        opacity: 0.3
      },
      _react2.default.createElement("path", {
        d:
          "m79.96 66.18v34.76c0 7.29-5.93 13.21-13.21 13.21h-3.28c-7.29 0-13.21-5.93-13.21-13.21v-34.76h29.7m4-4h-37.71v38.76c0 9.51 7.71 17.21 17.21 17.21h3.28c9.51 0 17.21-7.71 17.21-17.21v-38.76h0.01z",
        fill: "#1F0B08"
      })
    ),
    _react2.default.createElement("line", {
      x1: 65.1,
      x2: 65.1,
      y1: 62.18,
      y2: 96.32,
      clipPath: "url(#a)",
      fill: "none",
      opacity: 0.3,
      stroke: "#1F0B08",
      strokeMiterlimit: 10,
      strokeWidth: 4
    }),
    _react2.default.createElement(
      "radialGradient",
      {
        id: "c",
        cx: 32.902,
        cy: 63.101,
        r: 19.444,
        gradientTransform: "matrix(.9791 0 0 .9301 -4.964 6.472)",
        gradientUnits: "userSpaceOnUse"
      },
      _react2.default.createElement("stop", {
        stopColor: "#ED0E00",
        offset: 0.0051082
      }),
      _react2.default.createElement("stop", {
        stopColor: "#ED1709",
        stopOpacity: 0.8431,
        offset: 0.1455
      }),
      _react2.default.createElement("stop", {
        stopColor: "#ED2F23",
        stopOpacity: 0.582,
        offset: 0.3791
      }),
      _react2.default.createElement("stop", {
        stopColor: "#ED554C",
        stopOpacity: 0.2513,
        offset: 0.6751
      }),
      _react2.default.createElement("stop", {
        stopColor: "#ED7770",
        stopOpacity: 0,
        offset: 0.9
      })
    ),
    _react2.default.createElement("circle", {
      cx: 27.25,
      cy: 65.16,
      r: 17.5,
      fill: "url(#c)"
    }),
    _react2.default.createElement(
      "radialGradient",
      {
        id: "b",
        cx: 107.97,
        cy: 63.101,
        r: 19.444,
        gradientTransform: "matrix(.9791 0 0 .9301 -4.964 6.472)",
        gradientUnits: "userSpaceOnUse"
      },
      _react2.default.createElement("stop", {
        stopColor: "#ED0E00",
        offset: 0.0051082
      }),
      _react2.default.createElement("stop", {
        stopColor: "#ED1709",
        stopOpacity: 0.8431,
        offset: 0.1455
      }),
      _react2.default.createElement("stop", {
        stopColor: "#ED2F23",
        stopOpacity: 0.582,
        offset: 0.3791
      }),
      _react2.default.createElement("stop", {
        stopColor: "#ED554C",
        stopOpacity: 0.2513,
        offset: 0.6751
      }),
      _react2.default.createElement("stop", {
        stopColor: "#ED7770",
        stopOpacity: 0,
        offset: 0.9
      })
    ),
    _react2.default.createElement("circle", {
      cx: 100.75,
      cy: 65.16,
      r: 17.5,
      fill: "url(#b)"
    }),
    _react2.default.createElement("path", {
      d:
        "m14 89.57c-4.05 0-7.2-3.96-7.2-7.37 0-2.37 1.06-5.11 2.52-8.9l0.66-1.72c1.14-2.99 2.78-6.21 3.93-8.35 1.02 2.11 2.53 5.27 3.94 8.43 2.6 5.83 3.35 8.19 3.35 10.54 0 3.41-3.15 7.37-7.2 7.37z",
      fill: "#A3E2FF"
    }),
    _react2.default.createElement("path", {
      d:
        "m13.85 66.59c0.82 1.72 1.74 3.7 2.62 5.69 2.5 5.6 3.22 7.83 3.22 9.93 0 2.66-2.54 5.87-5.7 5.87s-5.7-3.21-5.7-5.87c0-2.09 1.01-4.72 2.42-8.36 0.21-0.55 0.44-1.13 0.66-1.73 0.72-1.86 1.62-3.81 2.48-5.53m0.15-6.59s-3.43 5.82-5.42 11.05c-1.82 4.78-3.28 8.13-3.28 11.15 0 4.32 3.89 8.87 8.7 8.87 4.8 0 8.7-4.55 8.7-8.87 0-2.72-0.88-5.31-3.48-11.15-2.49-5.57-5.22-11.05-5.22-11.05z",
      fill: "#38A9FF"
    }),
    _react2.default.createElement("path", {
      d:
        "m21.79 41.99c-4.05 0-7.2-3.96-7.2-7.37 0-2.37 1.05-5.11 2.51-8.89l0.66-1.72c1.14-2.99 2.78-6.21 3.93-8.35 1.02 2.11 2.53 5.27 3.94 8.43 2.6 5.83 3.35 8.19 3.35 10.54 0.01 3.4-3.13 7.36-7.19 7.36z",
      fill: "#A3E2FF"
    }),
    _react2.default.createElement("path", {
      d:
        "m21.65 19.01c0.81 1.72 1.74 3.7 2.62 5.69 2.5 5.6 3.22 7.83 3.22 9.93 0 2.66-2.54 5.87-5.7 5.87s-5.7-3.21-5.7-5.87c0-2.09 1.01-4.72 2.42-8.36 0.21-0.55 0.44-1.13 0.66-1.72 0.71-1.87 1.62-3.82 2.48-5.54m0.14-6.59s-3.43 5.82-5.42 11.05c-1.82 4.78-3.27 8.12-3.27 11.15 0 4.32 3.89 8.87 8.7 8.87s8.7-4.55 8.7-8.87c0-2.72-0.88-5.31-3.48-11.15-2.49-5.58-5.23-11.05-5.23-11.05z",
      fill: "#38A9FF"
    }),
    _react2.default.createElement("path", {
      d:
        "m116 74.99c-5.64 0-10.01-5.51-10.01-10.25 0-3.29 1.46-7.07 3.48-12.31 0.29-0.76 0.6-1.55 0.91-2.37 1.65-4.34 4.13-9.08 5.78-12.05 0.8 1.52 1.89 3.74 3.28 6.85 3.86 8.67 6.56 16.84 6.56 19.88 0 4.74-4.37 10.25-10 10.25z",
      fill: "#A3E2FF"
    }),
    _react2.default.createElement("path", {
      d:
        "m116.09 42.37c0.46 0.96 0.96 2.06 1.52 3.31 4.19 9.41 6.39 16.71 6.39 19.06 0 3.74-3.57 8.25-8 8.25-4.44 0-8.01-4.51-8.01-8.25 0-2.92 1.4-6.55 3.34-11.59 0.3-0.77 0.6-1.56 0.91-2.38 1.08-2.83 2.53-5.84 3.85-8.4m0.07-8.38s-4.9 8.13-7.65 15.36c-2.51 6.6-4.52 11.22-4.52 15.39 0 5.96 5.38 12.25 12.01 12.25s12-6.29 12-12.25c0-3.75-3.14-12.63-6.73-20.69-3.44-7.71-5.11-10.06-5.11-10.06z",
      fill: "#38A9FF"
    })
  );
}

exports.default = Hot;
